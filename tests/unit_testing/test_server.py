...
from Python_Testing.server import POINTS_PER_PLACE
...
@pytest.mark.parametrize("places", [-1, 0, 1, 6, 7, 8, 9, 12, 13])
def test_purchase_places(mocker, places, client, competitions, clubs):
    mocker.patch.object(server, 'competitions', competitions)
    mocker.patch.object(server, 'clubs', clubs)

    # 1) club et comp > 12
    if places in [-1, 0, 1, 7, 12, 13]:
        response = client.post('/purchasePlaces',
                               data=dict(places=places,
